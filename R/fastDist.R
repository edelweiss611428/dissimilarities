...
#'
#' @usage fastDist(X, method = "euclidean", diag = F, upper = F, p = 2)
#'
...
#'                times = 100)
#' #Check if equal
#' all.equal(as.vector(stats::dist(x, method = "minkowski", p = 5)), as.vector(fastDist(x, method = "minkowski", p = 5)))
#'
#' @author Minh Long Nguyen \email{edelweiss611428@gmail.com}
#' @export


fastDist = function(X, method, diag, upper, p = 2L){
  ...
  return(dissimilarities:::.fastDistCpp(X, method, diag, upper, p))
